...
Everything that runs the agent — the CLI, the background worker, the eval
harness — goes through :func:`run_agent`. It yields events instead of printing,
...
def _summarize_tool_result(name: str, content: str, ok: bool) -> str:
    ...
    if name == "search_schema":
        return f"{content.count('Table: ')} candidate tables"
    return f"{len(content)} chars"
