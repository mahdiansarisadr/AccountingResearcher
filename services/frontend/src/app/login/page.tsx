"use client";

import { useEffect } from "react";
import { useRouter } from "next/navigation";

import { getMe, loginUrl } from "@/lib/api";

export default function LoginPage() {
  const router = useRouter();

  useEffect(() => {
    getMe()
      .then(() => router.replace("/"))
      .catch(() => undefined);
  }, [router]);

  return (
    <main className="flex min-h-full flex-col items-center justify-center px-6">
      <div className="w-full max-w-md rounded-2xl bg-paper-raised p-10 shadow-sm ring-1 ring-black/5">
        <p className="text-xs uppercase tracking-[0.2em] text-ink-faint">Internal</p>
        <h1 className="mt-3 font-serif text-3xl leading-tight">
          Accounting Research Assistant
        </h1>
        <p className="mt-3 text-sm leading-6 text-ink-muted">
          Sign in with your company Google account. Access is restricted to that
          domain; there is no password and no invitation list.
        </p>
        <a
          href={loginUrl()}
          className="mt-8 flex items-center justify-center gap-3 rounded-xl bg-ink px-4 py-3 text-sm font-medium text-paper hover:bg-ink-muted"
        >
          <GoogleMark />
          Continue with Google
        </a>
      </div>
    </main>
  );
}

function GoogleMark() {
  return (
    <svg width="18" height="18" viewBox="0 0 48 48" aria-hidden="true">
      <path
        fill="#FFC107"
        d="M43.6 20.5H42V20H24v8h11.3C33.7 32.7 29.3 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3.1 0 5.8 1.2 8 3.1l5.7-5.7C34.2 6.1 29.4 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.3-.1-2.5-.4-3.5z"
      />
      <path
        fill="#FF3D00"
        d="M6.3 14.7l6.6 4.8C14.7 16 19 12 24 12c3.1 0 5.8 1.2 8 3.1l5.7-5.7C34.2 6.1 29.4 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"
      />
      <path
        fill="#4CAF50"
        d="M24 44c5.2 0 10-2 13.6-5.2l-6.3-5.3C29.2 35.1 26.7 36 24 36c-5.3 0-9.7-3.3-11.3-7.9l-6.5 5C9.6 39.6 16.3 44 24 44z"
      />
      <path
        fill="#1976D2"
        d="M43.6 20.5H42V20H24v8h11.3c-1.1 3.2-3.5 5.8-6.7 7.5l6.3 5.3C38.2 37.3 44 32 44 24c0-1.3-.1-2.5-.4-3.5z"
      />
    </svg>
  );
}
